...
def heuristic_strength(df: pd.DataFrame) -> np.ndarray:
    score = df["course"].map(COURSE_PRIOR).fillna(0).astype(float)
    ...
    components = [
        ("class_score", 0.42, False), ("global_win_pt", 0.52, False),
        ("global_in2nd", 0.23, False), ("local_win_pt", 0.18, False),
        ("local_in2nd", 0.12, False), ("motor_in2nd", 0.25, False),
        ("boat_in2nd", 0.08, False), ("aveST", 0.32, True),
        ("display_time", 0.34 + 0.10 * roughness, True),
        ("start_display_st", 0.30 + 0.10 * roughness, True),
    ]
    for col, weight, reverse in components:
        if col in df:
            score = score + weight * zscore(df[col], reverse=reverse).fillna(0)
    score = score - 0.18 * pd.to_numeric(df.get("F", 0), errors="coerce").fillna(0)
    score = score - 0.05 * pd.to_numeric(df.get("L", 0), errors="coerce").fillna(0)
