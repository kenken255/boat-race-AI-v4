...
st.set_page_config(page_title="BOAT RACE AI v4", page_icon="🚤", layout="centered", initial_sidebar_state="collapsed")
...
st.title("🚤 BOAT RACE AI v4")
st.caption("Web自動取得 + 市場×AI評価 + 仮想レース + 結果照合/自己改善。自動投票は行いません。")

main_tab, learn_tab, info_tab = st.tabs(["🎯 予想", "🧠 学習", "⚙️ 情報"])
...
with main_tab:
    ...
    if bundle:
        ...
        strengths = model_strength(df, trained) if trained else heuristic_strength(df)
        ...
        st.caption(f"モデル: {model_kind} / 統合: 解析{(1-sim_weight)*100:.0f}% + 仮想レース{sim_weight*100:.0f}%")
